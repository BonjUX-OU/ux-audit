...
export async function POST(request: Request) {
  try {
    ...
    const isValidInputs = await ValidatePayloadUserSignup(body);
    if (!isValidInputs) {
      return NextResponse.json({ error: VALIDATE_PAYLOAD_ERROR_MESSAGE }, { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ data: newUser, message: "User has been created successfully." }), {
      status: 200,
    });
    ...
    // const successResponse = new Response(JSON.stringify({ message: NEW_REGISTRATION_SUCCESS_MESSAGE }), {
    //   status: 200,
    // });

    // successResponse.headers.append(
    //   "Set-Cookie",
    //   `register_session=1; HttpOnly; Path=/; Max-Age=900; SameSite=Lax${
