...
export default function DashboardPage() {
  ...
  // We only have 3 steps now, each corresponds to a route
  ...
  // Create an untitled project using the current user as owner
  ...
  async function handleCreateAnalysis(e: FormEvent) {
    ...
    let newProjectId = "";
    if (projectId === "all") {
      // If user is on "All Reports", create an untitled project to hold this new analysis
      try {
    ...
    // Open the modal and start the process
    ...
    try {
      ...
      await step2Res.json(); // not used, but presumably { success: true }
      setAnalysisSteps((prev) =>
        prev.map((s, i) => (i === 1 ? { ...s, status: "done" } : s))
      );

      // -------------------------------
      // STEP 3: Final GPT Analysis & Storage
      // -------------------------------
      setAnalysisSteps((prev) =>
      ...
      setFinalReportId((await storeRes.json())._id);
      setAnalysisSteps((prev) =>
      ...
      // Mark whichever step is currently in "in-progress" to "error"
      setAnalysisSteps((prev) => {
                ...
                <div className="space-y-1 mt-2">
                  {projects.map((project) => (
                    <button
                      ...
                      className={cn(
                        "flex items-center w-full text-left px-3 py-2 rounded-md transition-all duration-200",
