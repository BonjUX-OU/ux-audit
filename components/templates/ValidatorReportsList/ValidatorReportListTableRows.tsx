...
import { Calendar, Check, ImagePlus, SendToBack, Trash2, UserPlus } from "lucide-react";
...
type ValidatorReportListTableRowsProps = {
  ...
  handleSendBackReport: (reportId: string) => void;
        ...
        {report.status === ReportStatus.InReview && (
          <>
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleSendBackReport(report._id!)}
              className="h-8 w-8 p-0 text-gray-400 hover:text-[#B04E34] transition-colors duration-200 [&_svg]:size-5">
              <SendToBack />
